...
- Playthrough bug re-test pending (user found 2026-07-26, fix committed on main as "fix: player spawns at the active layer's height"): the player rendered one layer below the game logic on levels whose start layer isn't array index 0 (ruins' forest sits above a basement layer with ids '-1','0','1','2') — TS's `levelSceneBuilder.ts:623-624` per-scene-build Y init was unported. The reported "floors/ceilings visible where they shouldn't be" is presumed the same wrong-vantage root cause. If the user still sees wrong planes from the CORRECT height after re-testing the rebuilt bundle, that's a distinct bug — next suspects: coplanar layer boundaries (layer N ceiling shares a plane with layer N+1 floor since LAYER_HEIGHT == WALL_HEIGHT) and the per-cell forced-ceiling rules on non-top layers.
