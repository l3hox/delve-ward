...
- Playthrough re-test pending (fixes committed on main 2026-07-26, bundle rebuilt): (1) "fix: player spawns at the active layer's height" — TS's `levelSceneBuilder.ts:623-624` per-scene-build Y init was unported, so levels whose start layer isn't array index 0 rendered the player a layer low. (2) "fix: culling frustum matches the cropped camera view" — Bevy 0.19's `update_frusta` ignores `sub_camera_view`, so our HUD crop (which *extends* the frame 20% below and 2.5% per side past the base frustum) had everything rendered in those bands frustum-culled: floor tiles vanished at the screen bottom, and culled meshes exposed coplanar geometry behind them (layer N's ceiling shares its plane with layer N+1's floor), which read as "floors/ceilings visible where they shouldn't be". `zones::sync_cropped_frusta` now rebuilds each cropped camera's `Frustum` from `computed.clip_from_view` between `UpdateFrusta` and `CheckVisibility`. (3) "fix: stairs transition to their target stair" — `on_player_moved` passed the stepped-on stair's own id to `begin_stair`, so every staircase faded and respawned the player in front of the stair they'd just entered; it now resolves the stair entity's `target` (`transitionSystem.ts:47`).
