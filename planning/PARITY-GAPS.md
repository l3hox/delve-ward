...
| dungeon.ts | Partial | `dungeon.rs` | Phase 3/5 | See Top Findings stair bug. Per-cell zone tagging, ramp wall/floor suppression, pit-trap floor toggling, and hollow-layer (`openBottom`/`openTop`) detection (`VerticalOpenness`, auto-detect from adjacent layers + area overrides) are in `dungeon.rs`; the zone-boundary half-tile split is still unported (disclosed-gap note below). |
